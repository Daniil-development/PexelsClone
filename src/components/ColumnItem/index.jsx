...
const ColumnItem = ({ item }) => {
  ...
  const [liked, setLiked] = useState(
    JSON.parse(localStorage.getItem("PhotoLikes")).includes(item.id),
  );
    ...
    >
      ...
      >
        <img
          ...
          src={item.src.original + "?auto=compress&cs=tinysrgb&dpr=1&w=500"}
          alt={item.alt}
        />
        ...
        {hover && (
          <div className={styles.buttonsContainer}>
            ...
            <a
              ...
              href={`${item.src.original}?cs=srgb&dl=pexels-${item.photographer
                .toLowerCase()
                .replaceAll(" ", "-")}-${item.id}.jpg&fm=jpg`}
              title="Download"
            >
              <DownloadIcon />
            </a>
            ...
            >
              {liked === true ? <LikedIcon /> : <NotLikedIcon />}
            </button>
          </div>
        )}
      </article>
